...
#' @export
#
fit_hal_single_lambda <- function(X,
                    Y,
                    degrees = NULL,
                    fit_type = c("origami", "glmnet"),
                    n_folds = 10,
...
#' @export
#
fit_hal_constraint_form <- function(X,
                    Y,
                    degrees = NULL,
                    fit_type = c("origami", "glmnet"),
                    n_folds = 3,
  ...
  if (fit_type == "glmnet") {
    # just use the standard implementation available in glmnet
    hal_lasso <- glmnet::cv.glmnet(x = x_basis, y = Y, nfolds = n_folds, lambda.min.ratio = 1e-6, ...)
    coefss <- stats::coef(hal_lasso, hal_lasso$lambda) # extract all beta in the lambda grid
    l1_norms <- colSums(abs(as.matrix(coefss)))
